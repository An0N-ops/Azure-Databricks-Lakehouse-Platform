...
variable "vnet_cidr" {
  type        = string
  description = "CIDR block for the platform Virtual Network. Must be unique per environment to avoid peering overlap."
...
variable "enable_nat_gateway" {
  type        = bool
  description = "Provision a NAT gateway for Databricks subnet egress. Disable only to save cost in ephemeral dev environments."
  default     = true
}
...
variable "storage_network_default_action" {
  type        = string
  description = "Storage firewall default action. Set Deny for prod lockdown (subnets are pre-allowed)."
  default     = "Allow"
...
variable "key_vault_network_default_action" {
  type        = string
  description = "Key Vault firewall default action. Set Deny for prod lockdown (subnets are pre-allowed)."
  default     = "Allow"
...
variable "key_vault_purge_protection_enabled" {
  type        = bool
  description = "Enable Key Vault purge protection. Disable for ephemeral dev to allow teardown."
  default     = true
}

variable "unity_catalog_data_owner_group" {
  type        = string
  description = "Entra group (e.g., data-engineers) granted USE CATALOG / schema privileges. Empty disables grants."
  default     = ""
}
