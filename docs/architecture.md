...
- **Silver (`cleansed`)**: conformed, validated entities. Schema validation, normalization, SCD Type 1/2 tracking, and Lakeflow quality expectations.
...
Transformations use Lakeflow Spark Declarative Pipelines (LSDP, `from pyspark import pipelines as dp`):

- **Declarative ETL**: flow dependencies authored as data (PySpark `dp.table` /
  `dp.temporary_view` / `dp.materialized_view` decorators and `dp.create_auto_cdc_flow`),
  reviewable in pull requests.
- **Data quality expectations**: `dp.expect`, `dp.expect_or_drop`, `dp.expect_or_fail`
  gate promotions between layers.
- **Auto-maintenance**: Lakeflow handles `OPTIMIZE`, Z-Ordering, and `VACUUM`.

### Bronze Ingestion Framework

Bronze is ingested with Auto Loader streaming into append-only Delta tables. The landing sources and quality contracts are declared as data in `pipelines/energy/bronze_manifest.json` — one table spec per entity — and rendered by the shared Lakeflow notebook `notebooks/bronze/ingest_energy.py`:
...
- **Quality policy**: expectations at the Bronze-to-Silver boundary use the ADR-005 **retain** policy — violating raw rows are preserved and flagged in the Lakeflow event log, never silently dropped.
...
- **SCD upserts**: each Silver table is the target of `dp.create_auto_cdc_flow`,
  sequenced by `_ingested_at` (SCD Type 1 by default; `customers` and `assets`
  opt into SCD Type 2 via `track_history_column_list`), so reprocessing Bronze
  is idempotent. Rows with null keys are ignored by the upsert but remain
  visible in Bronze.
...
- **Dimensions**: conformed Silver entities are registered as `dim_*` streaming
  tables with AUTO CDC upserts (`dp.create_auto_cdc_flow`, SCD Type 1) over a
  temporary view of Silver's Change Data Feed, so they stay in sync with
  Silver's updates incrementally; the manifest's foreign-key declarations make
  the star schema reviewable as data. `dim_date` is generated from a declared
  `date_range` via `notebooks/shared/gold_manifest.py` (`date_dimension_rows`),
  sharing the `YYYYMMDD` integer key used by the facts.
